...
interface OCPMIMDData {
  message: string
  svg_content: string
  status: string
}

interface ViewOCPMIMDProps {
  ocelIndex: number
}

function ViewOCPMIMD({ ocelIndex }: ViewOCPMIMDProps) {
  const [ocpmImdData, setOcpmImdData] = useState<OCPMIMDData | null>(null)
  ...
  const handleDiscover = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/discover_ocpm_imd', {
        method: 'POST',
        body: formData,
      })
      ...
      if (result.status === 'success') {
        setOcpmImdData(result)
        setIsInitialized(false) // Reset initialization flag for new discovery
      } else {
        setError(result.message || 'OCPM IMD Discovery failed')
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (ocpmImdData && !isInitialized && svgContainerRef.current) {
  ...
  }, [ocpmImdData, isInitialized])

  return (
    <div className="discovery-component">
      <h3 className="component-title">Inductive Miner Directly-Follows (IMD)</h3>
          ...
          {ocpmImdData && (
            ...
            >
              <div
                ...
                dangerouslySetInnerHTML={{ __html: ocpmImdData.svg_content }}
              />
            </div>
          )}
        </div>
            ...
            {ocpmImdData ? (
              <div className="algorithm-metrics">
                <div className="metric-section">
                  <div className="section-header">OCPM IMD Model</div>
