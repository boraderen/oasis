...
class ServiceSmokeTests(unittest.TestCase):
    ...
    def test_dfg_update_supports_top_k_and_manual_modes(self) -> None:
        exploration = build_log_exploration(LOG_PATH, "basic_log.xes")
        ...
        self.assertEqual(top_k["kept_variant_count"], 1)
    ...
    def test_discovery_and_conformance_smoke(self) -> None:
        discovery = discover_process_model(LOG_PATH, "basic_log.xes", "inductive", {"noise_threshold": 0.2})
        self.assertEqual(discovery["status"], "success")
        self.assertTrue(discovery["svg_content"])
        self.assertTrue(discovery["pnml_content"])

        log_log = conformance_log_log(LOG_PATH, "basic_log.xes", LOG_PATH, "basic_log.xes")
        self.assertIn("log1_insights", log_log)
        self.assertIn("log2_insights", log_log)

        conformance = conformance_log_model(LOG_PATH, "basic_log.xes", MODEL_PATH, "normative-model.pnml")
        ...
        first_variant = build_log_exploration(LOG_PATH, "basic_log.xes")["available_variants"][0]["activities"]
    ...
    def test_ocel_and_autopm_smoke(self) -> None:
        ocel = explore_ocel(OCEL_PATH, "example_log.jsonocel")
        ...
        ocpm = discover_ocpm(OCEL_PATH, "example_log.jsonocel", "im")
        ...
        flattened = flatten_ocel_to_event_log(OCEL_PATH, "example_log.jsonocel", first_object_type)
